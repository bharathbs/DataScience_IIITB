...
#	Import the Bollywood data set in Rstudio in a variable named bollywood
  ...
  bollywood <- read.csv("bank.csv",stringsAsFactors = F)
  View(bollywood)
...
# Store the movie name in variable named top_moviebo
...
# Store the movie name in variable named top_2_movie
  ...
  # Trying to sort the collection and pick the second best one.
...
# Write command/s to find out how many movies are in Flop, Average, Hit and Superhit categories in the entire Bollywood data set.
	...
	movieCategories<-factor(bollywood$Verdict)
  summary(movieCategories)
...
# Store the names of 4 movies in same sequence in movie_result vector
  ...
  #We can write a seperate function to find out the maximum collection Names.Here we are using anonymous function within sapply.
  
  movie_result <- sapply(c("Ocollection","Wcollection","Fwcollection","Tcollection"), function(collectionNames) bollywood$Movie[which.max((bollywood[,collectionNames]))])
